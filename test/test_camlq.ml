...
let assert_parse_success json_input expected_output =
  ...
  | PR_Success ast ->
      ...
      if result = expected_output then
        ...
        Printf.printf "✗ FAIL: %s\nExpected: %s\nGot: %s\n" json_input expected_output result
  | PR_Error msg ->
      Printf.printf "✗ FAIL: %s\nError: %s\n" json_input msg
...
let test_objects () =
  ...
  assert_parse_success "{\"nested\":{\"inner\":42}}" "{\"nested\": {\"inner\": 42.}}"
...
let test_complex_json () =
  ...
  let expected = "{\"name\": \"John Doe\", \"age\": 30., \"is_student\": false, \"courses\": [\"Math\", \"Science\", \"History\"], \"address\": {\"street\": \"123 Main St\", \"city\": \"Anytown\", \"zip\": \"12345\"}}" in
...
let test_ast_to_string () =
  ...
  let expected = "{\"name\": \"Alice\", \"age\": 25., \"active\": true, \"tags\": [\"dev\", \"ocaml\"]}" in
  let result = to_string obj in
  
  if result = expected then
    Printf.printf "✓ PASS: AST to_string\n"
  else
